...
function createWindow() {
    mainWindow = new BrowserWindow({
        width: 800,
        height: 600,
        autoHideMenuBar: true, // masquer la barre de menu par default de la fenetre electron 
        webPreferences: {
...
// fonction qui permet d'ouvrir la fenetre "accueil" sur evenement openAccueil
ipcMain.on('open-accueil', (event)=> {
    const accueilWindow = new BrowserWindow({
        width: 550,
        height: 500,
        title: "Accueil",
        modal: true, // bloque l'acces aux autres fenetres de l'app
        parent: mainWindow || undefined, // pour une fenetre modale, specifier la fenetre parent
        show: false,
        autoHideMenuBar: true, // masquer la barre de menu par default de la fenetre electron 
        webPreferences:{
            preload: path.join(__dirname, '../preload/preload.js'),
    ...
    // Attendre le chargement complet de la fenetre avant de l'afficher
    accueilWindow?.webContents.on('did-finish-load', () => {
        accueilWindow.show()
    })

    // Chargement de la route Vue dans la nouvelle fenetre
    accueilWindow.loadURL('http://localhost:5173/#/accueil')
})


// fonction qui permet d'ouvrir la fenetre "todo" sur evenement open-Todo
ipcMain.on('open-todo', (event)=> {
    ...
    // Chargement de la route Vue dans la nouvelle fenetre
    todoWindow.loadURL('http://localhost:5173/#/todo')
})