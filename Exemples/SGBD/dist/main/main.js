...
function requireLib$3() {
  ...
  (function(exports$1) {
    ...
    exports$1.raw = exports$1.format = exports$1.escape = exports$1.arrayToList = exports$1.bufferToString = exports$1.objectToValues = exports$1.escapeId = exports$1.dateToString = void 0;
    ...
    exports$1.dateToString = dateToString;
    const escapeId = (value, forbidQualified) => {
      if (Array.isArray(value)) {
        const length = value.length;
        const parts = new Array(length);
        for (let i = 0; i < length; i++)
          parts[i] = (0, exports$1.escapeId)(value[i], forbidQualified);
        return parts.join(", ");
      }
    ...
    const objectToValues = (object, timezone) => {
      ...
      for (let i = 0; i < keysLength; i++) {
        ...
        if (sql.length > 0)
          sql += ", ";
        sql += (0, exports$1.escapeId)(key);
        sql += " = ";
        sql += (0, exports$1.escape)(value, true, timezone);
      }
      return sql;
    };
    exports$1.objectToValues = objectToValues;
    const bufferToString = (buffer) => `X${escapeString(buffer.toString("hex"))}`;
    exports$1.bufferToString = bufferToString;
    const arrayToList = (array, timezone) => {
      ...
      for (let i = 0; i < length; i++) {
        const value = array[i];
        if (Array.isArray(value))
          parts[i] = `(${(0, exports$1.arrayToList)(value, timezone)})`;
        else
          parts[i] = (0, exports$1.escape)(value, true, timezone);
      }
      return parts.join(", ");
    };
    exports$1.arrayToList = arrayToList;
    const escape = (value, stringifyObjects, timezone) => {
      ...
      switch (typeof value) {
        ...
        case "object": {
          if (isDate(value))
            return (0, exports$1.dateToString)(value, timezone || "local");
          if (Array.isArray(value))
            return (0, exports$1.arrayToList)(value, timezone);
          if (node_buffer_1.Buffer.isBuffer(value))
            return (0, exports$1.bufferToString)(value);
          if (value instanceof Uint8Array)
            return (0, exports$1.bufferToString)(node_buffer_1.Buffer.from(value));
          if (hasSqlString(value))
            return String(value.toSqlString());
          if (!(stringifyObjects === void 0 || stringifyObjects === null))
            return escapeString(String(value));
          if (isRecord(value))
            return (0, exports$1.objectToValues)(value, timezone);
          return escapeString(String(value));
        }
        case "string":
    ...
    const format = (sql, values, stringifyObjects, timezone) => {
      ...
      while (valuesIndex < length && placeholderPosition !== -1) {
        ...
        if (placeholderLength === 2)
          escapedValue = (0, exports$1.escapeId)(currentValue);
          ...
          if (setIndex !== -1 && setIndex <= placeholderPosition && hasOnlyWhitespaceBetween(sql, setIndex, placeholderPosition) && !hasSqlString(currentValue) && !Array.isArray(currentValue) && !node_buffer_1.Buffer.isBuffer(currentValue) && !(currentValue instanceof Uint8Array) && !isDate(currentValue) && isRecord(currentValue)) {
            escapedValue = (0, exports$1.objectToValues)(currentValue, timezone);
            setIndex = findSetKeyword(sql, placeholderEnd);
          } else
            escapedValue = (0, exports$1.escape)(currentValue, true, timezone);
        } else
          escapedValue = (0, exports$1.escape)(currentValue, stringifyObjects, timezone);
...
function requirePacket() {
  ...
  const NativeBuffer = require$$0$2.Buffer;
...
function requirePromise() {
  ...
  (function(exports$1) {
    ...
    exports$1.Connection = PromiseConnection;
    exports$1.PoolConnection = PromisePoolConnection;
...
class ParticipantService {
  ...
  async chargerParticipants() {
    try {
      const query2 = `SELECT * FROM participant`;
      const rows = await this.db.query(query2);
      return rows.map((row) => new Participant({
  ...
  async ajouterParticipant(participant) {
    try {
      const query2 = `INSERT INTO participant (Matricule, Prenom, Nom, Genre, Niveau, Email, isActif) VALUES (?, ?, ?, ?, ?, ?, ?)`;
      ...
      await this.db.execute(query2, params);
    } catch (error) {
  ...
  async supprimerParticipant(matricule) {
    try {
      const query2 = `DELETE FROM participant WHERE Matricule = ?`;
      const result = await this.db.execute(query2, [matricule]);
  ...
  async modifierParticipant(updated) {
    try {
      ...
      const query2 = `UPDATE participant SET Prenom = ?, Nom = ?, Genre = ?, Niveau = ?, Email = ?, isActif = ? WHERE Matricule = ?`;
      const params = [
        updated.prenom,
        updated.nom,
        updated.genre,
      ...
      const result = await this.db.execute(query2, params);
      if (result.affectedRows === 0) {
...
electron.app.on("ready", async () => {
  ...
  try {
    await participantService.init();
    console.log("Service des participants initialisé avec la BD");
  } catch (error) {
    console.error("Erreur lors de l'initialisation du service :", error);
    electron.dialog.showErrorBox("Erreur d'initialisation", "Une erreur est survenue lors de l'initialisation du service de participants. Veuillez vérifier la console pour plus de détails.");
    electron.app.quit();
    return;
  }
...
electron.ipcMain.on("ajouter-participant", () => {
  const ajoutWindow = new electron.BrowserWindow({
    ...
    // Associer la fenêtre modale à la fenêtre principale
    show: false,
    webPreferences: {
      preload: path.join(__dirname, "../preload/preload.js"),
      contextIsolation: true
...
electron.ipcMain.handle("Canal-AjouterParticipant", async (event, participant) => {
  try {
